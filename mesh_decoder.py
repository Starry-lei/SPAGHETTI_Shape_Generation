...
class MeshSampler(Dataset):
    ...
    def init_in_out(self, total=6e5, max_trials=50) -> TS:
        split_size = int(total // 4) #  150000
        print("split_size",split_size)
        # exit()
        on_surface_points = self.get_surface_points(split_size)
        near_points_a = on_surface_points + torch.randn(on_surface_points.shape) * .007# 𝜎 = 0.01
        near_points_b = on_surface_points + torch.randn(on_surface_points.shape) * .02 # 𝜎 = 0.02
        ...
        # print("labels shape", labels.shape)# labels shape torch.Size([3, 150000])
        print("all_points shape", all_points.shape)# all_points shape torch.Size([4, 150000, 3])
        # print("inside_points shape", inside_points.shape)# inside_points shape torch.Size([150000, 3])
        # exit()
    ...
    def __init__(self, path, inside_outside=True, num_samples=6e5):
        ...
        self.mesh = mesh
        self.split = (2, 2, 2)
        self.points, self.labels = self.init_in_out(total=num_samples)# input data step 1
...
class MeshProjection(occ_inference.Inference, abc.ABC):
    ...
    def train_epoch(self, epoch: int):
        # print("show len(self.dl):",len(self.dl))# 30
        # exit()
        self.logger.start(len(self.dl), tag=f'{self.opt.tag}_projection')
        for data in self.dl:
            samples, labels = map(lambda x: x.to(self.device), data)
            # print("samples shape", samples.shape)# samples shape torch.Size([5000, 4, 3])
            # print("labels shape", labels.shape) # labels shape torch.Size([5000, 3])                       
            # exit()            
    ...
    def __init__(self, opt: options.Options, mesh_path: str, folder_out):
        ...
        print("show mesh_path:", mesh_path)# ./assets/mesh/example.obj
        self.dl: DataLoader = get_data_loader(mesh_path)# step 2
        # exit()
...
class MeshProjectionMid(MeshProjection):
    ...
    def train_iter_low(self, labels, samples_sdf, samples_gmm):
        ...
        print("show z shape:", z.shape)# show z shape:  torch.Size([1, 256])
        # exit()
    ...
    def train_iter_high(self, labels: T, samples_sdf: T, samples_gmm: T):
        print("enter the function of train_iter_high?")
        ...
        print("end of the function of train_iter_high?")
        return loss

    def train_iter(self, labels, samples_sdf, samples_gmm):
        print("See self.projection_type:", self.projection_type)# show self.projection_type: ProjectionType.LowProjection
        # exit()
    ...
    def save_projection(self, res=256, verbose=False):
        print("enter the function of save_projection?")
        z_d = self.mid_embeddings(torch.zeros(1, device=self.device, dtype=torch.int64)).view(1, self.opt.num_gaussians, -1)
        zh_base, gmms = self.model.decomposition_control.forward_mid(z_d)
        print("show zh_base shape:", zh_base.shape)# torch.Size([1, 16, 512])
        print("show gmms shape:", len(gmms))# show gmms shape: 4
        zh, attn_b = self.model.merge_zh(zh_base, gmms)
        numbers = self.get_new_ids(self.folder_name, 1)
        print("before the end of the function of save_projection?")
...
def main():
    for_parser = {'--model_name': {'default': 'chairs_large', 'type': str},
                  ...
                  '--source':  {'default': 'random', 'type': str, 'options': ('inversion', 'random', 'training')},
                  '--num_samples':  {'default': 1, 'type': int, 'help': 'relevant for random or training'}}
    args = myparse.parse(for_parser)
    opt = options.Options(tag=args['model_name'])

    save_latents= {}
    path_save= f'{opt.cp_folder}/{args["output_name"]}/latents'
    if not os.path.exists(path_save):
        os.makedirs(path_save)
    print("show path_save:", path_save)
...
if __name__ == '__main__':
    ...
    # python shape_inversion.py --model_name  chairs_large  --source training --mesh_path --num_samples 1
